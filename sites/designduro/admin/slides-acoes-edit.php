<?php
    ...
    function update($id, $titulo, $texto, $nome_atual){
        //echo "entrei na funcao de salvar com imagem";
        //exit();
        $sqlInsere = "UPDATE slides SET 
        titulo = '$titulo', texto = '$texto', imagem = '$nome_atual'
        WHERE
        id = $id";
        return update_db($sqlInsere);
    }

    function updateSemImagem($id, $titulo, $texto){
    ...
    function deletaArquivo($id){
        $sqlConsulta    = "SELECT imagem FROM slides WHERE id = $id";
        //exit();
    ...
    function uploadImg($id, $titulo, $texto, $imagem){
        ...
        if($tamanho < 512){ //se imagem for até 1MB envia
            ...
            if(move_uploaded_file($tmp,$pasta.$nome_atual)){
                if(deletaArquivo($id)){
                    ...
                    if(update($id, $titulo, $texto, $nome_atual)){
                        echo "<script type='text/javascript'>alert('Operação realizada com sucesso!'); window.location = 'slides.php';</script>";
                        exit();
                    }
                }
            } else {
    ...
    if(isset($imagem) && $imagem["name"] != ""){
        //echo "oi";
        //exit();
        uploadImg($id, $titulo, $texto, $imagem);
    } else {
        //echo "oi sem imagem";
        //exit();
        if(updateSemImagem($id, $titulo, $texto)){
            echo "<script type='text/javascript'>alert('Operação realizada com sucesso!'); window.location = 'slides.php';</script>";
            exit();
        }
    }
